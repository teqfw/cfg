...
- Treat the root repository and `ctx/` as separate Git repositories once `ctx/` is mounted.
...
Before starting work, fetch the repository and ensure the local branch in use matches its upstream version. When `ctx/` is mounted, apply the same check to its repository before changing it.
